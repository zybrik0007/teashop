...
export class ModalAdminComponent implements OnInit, OnChanges {
  ...
  transfer(event) {
    console.log(event)
    fromEvent(document.getElementById(event), 'mousedown').subscribe(el => {
  ...
  dragElement(elmnt) {
    ...
    if (document.getElementById(elmnt + '-head')) {
      /* if present, the header is where you move the DIV from:*/
      document.getElementById(elmnt + '-head').onmousedown = dragMouseDown;
    } else {
